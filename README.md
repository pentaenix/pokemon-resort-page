...
The public app intentionally keeps navigation compact. Top-level routes match the header in `src/components/Layout.jsx`:

- **Home** (`#/`): the resort lobby, project status, entry cards, and a clear “What is this?” about section.
- **Island Atlas** (`#/atlas`): 2D cork-board pins, 3D island viewport, media carousel, submodels, character/sprite planning, and gallery resources.
- **Compatibility** (`#/ontology`): the generation ontology with stable directional routes, focus mode, free-layout dragging, outward self-loops, large line hit boxes, and mobile route fallback.
- **Operations** (`#/board`): On-Flight Board, internal bugs, and community issue links.
- **Ideas & Milestones** (`#/milestones`): hub with links to the spark board and build timeline.
- **Docs** (`#/docs`): technical articles for contributors (`#/docs?article=slug`).
- **Source Guide** (`#/source`): repo structure and local update workflow.
- **Legal** (`#/legal`): full fan-project disclaimer and asset/credit stance.

Sub-routes (not in the header; linked from hubs and cross-links):

- **Spark board** (`#/ideas`): uncommitted idea cards and optional dossiers.
- **Build timeline** (`#/build`): vertical scrollable milestone roadmap centered on the current step.

Aliases: `#/issues` → Operations; `#/roadmap` → build timeline; `#/gallery`, `#/models`, and `#/characters` → Island Atlas; `#/research` and `#/concierge` → Island Atlas; `#/plan` → Ideas & Milestones hub.
...
- Idea board cards
- Docs hub metadata and article bodies
...
public/data/atlas-pins.json        Island Atlas 2D map pins and legend
...
public/data/ideas.json             spark board cards
public/data/docs.json              Docs hub index (categories and article cards)
public/data/docs-freshness.json    staleness badges (generated by validate:data)
public/docs/articles/              article bodies by category folder
public/ideas/articles/             optional idea dossiers
```

Atlas 3D POI coordinates are edited in the Operations Desk as `public/data/pois.json` (desk-only; not loaded by the public boot bundle). Legacy `research-pois.json` was split into `research.json` and `pois.json`.
