...
function tilePanel(state) {
  ...
  return `<section class="tpe-tile-layout">
    <aside class="tpe-tile-list"><input data-tpe-tile-search placeholder="Search tiles" value="${esc(state.search)}"><small>${matches.length > visibleTiles.length ? `Showing ${visibleTiles.length} of ${matches.length}. Search to narrow the list.` : `${matches.length} tiles`}</small><div>${visibleTiles.map((item) => `<button class="${item.resortTileId === tile.resortTileId ? 'active' : ''}" data-tpe-select-tile="${item.resortTileId}"><strong>${esc(item.name || item.key)}</strong><span>#${item.resortTileId}</span></button>`).join('')}</div></aside>
    <div class="tpe-panel tpe-inspector">
      <div class="tpe-section-head"><div class="tpe-tile-summary"><img src="/api/tile-packages/preview?file=${encodeURIComponent(state.fileName)}&tileId=${tile.resortTileId}" alt="" onerror="this.style.display='none'"><div><h3>${esc(tile.name)}</h3><p>Stable ID #${tile.resortTileId} · ${tile.width}×${tile.height} footprint</p></div></div><span class="tpe-status">${tile.animation?.type !== 'none' ? 'Animated' : 'Static'}</span></div>
...
function addPanel(state) {
  ...
  const bundleSummary = bundle ? `<div class="tpe-bundle-summary"><strong>${esc(bundle.name)}</strong><span>${bundle.materialCount} material${bundle.materialCount === 1 ? '' : 's'} · ${bundle.animatedMaterials.length} animated</span>${bundle.animatedMaterials.map((item) => `<small>${esc(item.material)}: ${item.frameCount} frames at ${item.frameDurationMs} ms</small>`).join('')}${bundle.glbAnimationClips?.length ? `<small>${bundle.glbAnimationClips.length} GLB motion clip${bundle.glbAnimationClips.length === 1 ? '' : 's'} will import at bind pose.</small>` : ''}</div>` : '';
  return `<section class="tpe-panel"><div class="tpe-section-head"><div><h3>Add tile</h3><p>Import a Pokemon Resort .tile bundle, textured GLB, PNG/JPG tile, or spritesheet.</p></div></div>
    <div class="tpe-drop"><input type="file" data-tpe-asset accept=".tile,.glb,.png,.jpg,.jpeg,application/zip,image/png,image/jpeg,model/gltf-binary"><strong>Choose tile asset</strong><span data-tpe-file-label>${state.asset ? esc(state.asset.name) : '.tile, GLB, PNG, JPG, or spritesheet'}</span></div>
    ${bundleSummary}
    <div class="tpe-form-grid">
    ...
    </div><div class="tpe-actions"><button class="btn primary" data-tpe-import>Add to pack</button></div>
    <p class="tpe-help">A .tile bundle keeps its GLB mesh, static materials, and material frame sequences together. Animated spritesheets remain available for flat tiles.</p>
  </section>`;
}
...
export async function openTilePackEditor(editor, { onSaved, log } = {}) {
  ...
  const state = { fileName: editor.tilePackage.fileName, document: payload.document, view: 'tiles', selectedTileId: payload.document.tiles[0]?.resortTileId, selectedPath: 0, search: '', asset: null, bundleInfo: null, busy: false };
  ...
  const save = async () => {
    try {
      ...
      editor.tilePackage = result.package; state.busy = false; message('Tile pack saved.', 'ok'); onSaved?.(result.package); log?.('Tile pack updated.', 'ok'); render();
    } catch (error) { state.busy = false; message(error.message, 'error'); }
  };

  const importTile = async () => {
    const file = state.asset; if (!file) return message('Choose an asset first.', 'error');
    try {
      ...
      const tile = { name: q('[data-tpe-add-name]').value.trim() || file.name.replace(/\.[^.]+$/, ''), tabId: q('[data-tpe-add-tab]').value, width: Number(q('[data-tpe-add-width]').value), height: Number(q('[data-tpe-add-height]').value), renderMode: q('[data-tpe-add-render]').value, tags: q('[data-tpe-add-tags]').value.split(',').map((tag) => tag.trim()).filter(Boolean), properties: state.bundleInfo?.properties || {}, collision: { mode: q('[data-tpe-add-collision]').value, autoApply: q('[data-tpe-add-collision]').value !== 'none' }, animation: { type: animationType, columns: Number(q('[data-tpe-add-cols]').value), rows: Number(q('[data-tpe-add-rows]').value), frameCount: Number(q('[data-tpe-add-count]').value), frameDurationMs: Number(q('[data-tpe-add-ms]').value), phase: 'global', loop: true } };
      const form = new FormData(); form.append('metadata', new Blob([JSON.stringify({ fileName: editor.tilePackage.fileName, tile })], { type: 'application/json' }));
      if (/\.tile$/i.test(file.name)) form.append('tileBundle', file, file.name);
      ...
      message('Adding tile and rebuilding the pack…');
      const result = await api('/api/tile-packages/tiles', { method: 'POST', body: form });
      editor.tilePackage = result.package; onSaved?.(result.package); log?.(`Added tile #${result.resortTileId}.`, 'ok');
      const refreshed = await api(`/api/tile-packages/authoring?file=${encodeURIComponent(editor.tilePackage.fileName)}`); state.document = refreshed.document; state.selectedTileId = result.resortTileId; state.view = 'tiles'; render();
  ...
  const bind = () => {
    ...
    overlay.querySelectorAll('[data-tpe-select-tile]').forEach((button) => button.onclick = () => { try { syncInspector(overlay, state); state.selectedTileId = Number(button.dataset.tpeSelectTile); render(); } catch (error) { message(error.message, 'error'); } });
    ...
    const asset = overlay.querySelector('[data-tpe-asset]'); if (asset) asset.onchange = async () => {
      state.asset = asset.files?.[0] || null; state.bundleInfo = null;
      const label = overlay.querySelector('[data-tpe-file-label]'); if (label && state.asset) label.textContent = state.asset.name;
      ...
      try {
        ...
        state.bundleInfo = inspected.bundle; render(); message('Tile bundle ready.', 'ok');
