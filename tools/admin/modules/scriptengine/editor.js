const ACTIONS = ['WAIT', 'FACE', 'FACE_PLAYER', 'FACE_AWAY', 'MOVE', 'WANDER', 'JUMP', 'TEXT', 'TEXT_FREE', 'POKEMON_INTERACTION_SESSION', 'CRY', 'EMOTICON'];
const VARIABLES = [
...
export function bindEditor(state, { api, render, log }) {
  ...
  document.querySelectorAll('[data-action-field]').forEach((input) => input.oninput = () => { const action = editor.script.actions[editor.selectedAction]; const key = input.dataset.actionField; if (key === 'text') action.text = input.value; else action[key] = input.type === 'number' ? Number(input.value) : input.value; });
