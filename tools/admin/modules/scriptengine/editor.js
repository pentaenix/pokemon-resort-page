const ACTIONS = ['WAIT', 'FACE', 'FACE_PLAYER', 'FACE_AWAY', 'MOVE', 'WANDER', 'JUMP', 'TEXT', 'TEXT_FREE', 'POKEMON_INTERACTION_SESSION', 'DISABLE_ATTEND', 'ENABLE_ATTEND', 'EXIT_INTERACTION', 'CRY', 'EMOTICON', 'PLAY_TILE_ANIMATION', 'TRANSITION_CLOSE', 'TRANSITION_OPEN', 'TELEPORT_TO_LINK', 'MOVE_PLAYER'];
const VARIABLES = [
...
const blankScript = (kind = 'interaction') => ({ id: 'new_script', kind, targetGates: kind === 'door' ? ['DOOR'] : ['POKEMON'], trigger: kind === 'door' ? 'MOVE_TOWARD' : kind === 'interaction' ? 'ACCEPT' : 'AUTONOMOUS', priority: 0, weight: 1, cooldownSeconds: 0, when: { allTags: [], noneTags: [] }, actions: [{ action: kind === 'door' ? 'PLAY_TILE_ANIMATION' : kind === 'interaction' ? 'TEXT_FREE' : 'WANDER', ...(kind === 'door' ? { value: 'open' } : {}) }] });
...
function actionInspectorFields(action) {
  ...
  if (type === 'PLAY_TILE_ANIMATION') fields.push(`<label>Door state<select data-action-field="value">${['open','close'].map((name) => `<option value="${name}" ${action.value === name ? 'selected' : ''}>${name}</option>`).join('')}</select></label>`);
  if (type === 'MOVE_PLAYER') fields.push(`<label>Direction<select data-action-field="direction">${['forward','south','north','east','west'].map((name) => `<option value="${name}" ${action.direction === name ? 'selected' : ''}>${name}</option>`).join('')}</select></label><label>Tiles<input data-action-field="tiles" type="number" min="1" value="${Number(action.tiles) || 1}"></label><label>Duration seconds<input data-action-field="durationSeconds" type="number" min="0.01" step="0.01" value="${Number(action.durationSeconds) || 0.25}"></label>`);
  return fields.join('');
}
...
export function bindEditor(state, { api, render, log }) {
  ...
  document.querySelectorAll('[data-script-field]').forEach((input) => input.oninput = () => { const key = input.dataset.scriptField; editor.script[key] = input.type === 'number' ? Number(input.value) : input.value; if (key === 'kind' && input.value === 'door') { editor.script.trigger = 'MOVE_TOWARD'; editor.script.targetGates = ['DOOR']; editor.selectedPath = editor.selectedPath.replace(/^interactions\//, 'doors/'); render(); } });
